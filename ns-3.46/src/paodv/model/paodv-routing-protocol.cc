 ...
 *
 * Based on
 *      NS-2 PAODV model developed by the CMU/MONARCH group and optimized and
 *      tuned by Samir Das and Mahesh Marina, University of Cincinnati;
 *
 *      PAODV-UU implementation by Erik Nordström of Uppsala University
 *      https://web.archive.org/web/20100527072022/http://core.it.uu.se/core/index.php/PAODV-UU
 *
 * Authors: Elena Buchatskaia <borovkovaes@iitp.ru>
 *          Pavel Boyko <boyko@iitp.ru>
 */

#include "paodv-routing-protocol.h"
...
NS_LOG_COMPONENT_DEFINE("PAodvRoutingProtocol");

namespace paodv
{
NS_OBJECT_ENSURE_REGISTERED(RoutingProtocol);

/// UDP Port for PAODV control traffic
const uint32_t RoutingProtocol::PAODV_PORT = 654;

/**
 * @ingroup paodv
 * @brief Tag used by PAODV implementation
 */
class DeferredRouteOutputTag : public Tag
{
  public:
    ...
    {
        static TypeId tid = TypeId("ns3::paodv::DeferredRouteOutputTag")
                                .SetParent<Tag>()
                                .SetGroupName("PAodv")
...
RoutingProtocol::RoutingProtocol()
    : m_rreqRetries(2),
      ...
      m_enableHello(false),
      m_enableBroadcast(true),            // default is true in PAODV
      m_ipv4(nullptr),
      m_socketAddresses(),
...
{
    static TypeId tid =
        TypeId("ns3::paodv::RoutingProtocol")
                             ...
                             Socket::SocketErrno& sockerr)
{
    ...
    if (m_socketAddresses.empty())
    {
        ...
        NS_LOG_LOGIC("No paodv interfaces");
        Ptr<Ipv4Route> route;
        return route;
    }
                            ...
                            const ErrorCallback& ecb)
{
    ...
    // PAODV is not a multicast routing protocol
    if (dst.IsMulticast())
    {
        return false;
    }
        ...
        if (m_ipv4->GetInterfaceForAddress(iface.GetLocal()) == iif)
        {
            if (dst == iface.GetBroadcast() || dst.IsBroadcast())
            {
                ...
                if (header.GetProtocol() == UdpL4Protocol::PROT_NUMBER)
                {
                    ...
                    if (udpHeader.GetDestinationPort() == PAODV_PORT)
                    {
                        // PAODV packets sent in broadcast are already managed
                        return true;
                    }
                }
                if (header.GetTtl() > 1)
...
RoutingProtocol::NotifyInterfaceUp(uint32_t i)
{
    ...
    {
        NS_LOG_WARN("PAODV does not work with more then one address per each interface.");
    }
    ...
    socket->Bind(InetSocketAddress(iface.GetLocal(), PAODV_PORT));
    ...
    socket->SetRecvCallback(MakeCallback(&RoutingProtocol::RecvAodv, this));
    socket->BindToNetDevice(l3->GetNetDevice(i));
    socket->Bind(InetSocketAddress(iface.GetBroadcast(), PAODV_PORT));
    socket->SetAllowBroadcast(true);
    socket->SetIpRecvTtl(true);
...
RoutingProtocol::NotifyTxError(WifiMacDropReason reason, Ptr<const WifiMpdu> mpdu)
{
    ...
    // Not every MAC TX error means a broken route in PAODV/AODV.
...
RoutingProtocol::NotifyAddAddress(uint32_t i, Ipv4InterfaceAddress address)
{
    ...
    {
        NS_LOG_LOGIC("PAODV does not work with more then one address per each interface. Ignore "
                     "added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress(uint32_t i, Ipv4InterfaceAddress address)
{
    ...
    {
        NS_LOG_LOGIC("Remove address not participating in PAODV operation");
    }
}
...
RoutingProtocol::LoopbackRoute(const Ipv4Header& hdr, Ptr<NetDevice> oif) const
{
    ...
    // returned when PAODV does not have a route; this causes the packet
    // to be looped back and handled (cached) in RouteInput() method
    // while a route is found. However, connection-oriented protocols
    // like TCP need to create an endpoint four-tuple (src, src port,
    // dst, dst port) and create a pseudo-header for checksumming.  So,
    // PAODV needs to guess correctly what the eventual source address
    // will be.
    //
    // For single interface, single address nodes, this is not a problem.
    // When there are possibly multiple outgoing interfaces, the policy
    // implemented here is to pick the first available PAODV interface.
    ...
    NS_ASSERT_MSG(rt->GetSource() != Ipv4Address(), "Valid PAODV source address not found");
...
RoutingProtocol::SendRequest(Ipv4Address dst)
{
    ...
    // ---------- PAODV neighbor selection (prefer FAR neighbors) ----------
    std::vector<Ipv4Address> priorNeighbors; // neighbors farther than threshold (prefer)
    std::vector<Ipv4Address> overheadNeighbors; // neighbors within threshold
    Ptr<Node> thisNode = nullptr;

    // Derive local node: prefer interface local address mapping if possible
    if (!m_socketAddresses.empty())
    {
        thisNode = GetNodeFromIpv4(m_socketAddresses.begin()->second.GetLocal());
    }
    if (thisNode == nullptr && m_ipv4)
    {
        thisNode = m_ipv4->GetObject<Node>();
    }
    if (thisNode == nullptr)
    {
        // If we can't map node, fallback to original broadcast behavior
        NS_LOG_WARN("PAODV: cannot map local node in SendRequest(); falling back to broadcast send.");
    }
    else
    {
        for (const auto &nb : m_nb.GetNeighbors())
        {
            ...
            if (!std::isfinite(d))
                continue;
            // **Paper behavior**: prefer farther neighbors as "prior" (use > threshold)
    ...
    // Selection helper: randomly pick up to bound from a vector (removes chosen elements)
    auto pickRandomSubset = [&](std::vector<Ipv4Address> &pool, std::vector<Ipv4Address> &out, uint32_t bound) {
        while (!pool.empty() && out.size() < bound)
        {
            uint32_t idx = m_uniformRandomVariable->GetInteger(0, static_cast<int>(pool.size() - 1));
            out.push_back(pool[idx]);
            // remove chosen element efficiently by swap-pop
            if (idx != pool.size() - 1)
            {
                pool[idx] = pool.back();
            }
    ...
    // Build selected list: prefer priorNeighbors, then overheadNeighbors to fill bound
    std::vector<Ipv4Address> selected;
    uint32_t bound = m_rreqBound;
    if (!priorNeighbors.empty())
    {
        pickRandomSubset(priorNeighbors, selected, bound);
    }
    if (selected.size() < bound && !overheadNeighbors.empty())
    {
        pickRandomSubset(overheadNeighbors, selected, bound);
    }

    // If nothing selected, fallback to broadcast (avoid strict drop)
    bool fallbackToBroadcast = selected.empty();

    // ---------------- send RREQs ----------------
    if (!fallbackToBroadcast)
    {
        // Unicast RREQ to each selected neighbor (on each PAODV socket)
        for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
        {
            ...
            // increment PAODV RREQ counter ONCE per interface attempt
            // This now matches AODV (1 broadcast = 1 RREQ count)
            ++m_rreqSentCount;
        }
    }
    else
    {
        // Original broadcast behavior as fallback (one per interface)
        for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
        {
            Ptr<Socket> socket = j->first;
            Ipv4InterfaceAddress iface = j->second;
            ...
            NS_LOG_DEBUG("Send RREQ with id " << rreqHeader.GetId() << " to socket (broadcast fallback)");
            ...
            Simulator::Schedule(MilliSeconds(m_uniformRandomVariable->GetInteger(0, 10)),
                                ...
                                packet,
                                destination);
            // increment counter for broadcast (we count one per socket)
            ++m_rreqSentCount;
        }
    }
...
{
    socket->SendTo(packet, 0, InetSocketAddress(destination, PAODV_PORT));
}
...
RoutingProtocol::RecvAodv(Ptr<Socket> socket)
{
    ...
    NS_LOG_DEBUG("PAODV node " << this << " received a PAODV packet from " << sender << " to "
                              << receiver);
    ...
    if (!tHeader.IsValid())
    {
        NS_LOG_DEBUG("PAODV message " << packet->GetUid() << " with unknown type received: "
                                     << tHeader.Get() << ". Drop");
        return; // drop
    }
    switch (tHeader.Get())
...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    ...
    // (reverse route creation / update) --- unchanged
    RoutingTableEntry toOrigin;
    ...
    // ---------------- PAODV neighbor classification (prefer farther neighbors) ----------------
    std::vector<Ipv4Address> priorNeighbors; // farther than threshold (preferred)
    std::vector<Ipv4Address> overheadNeighbors; // closer than or equal threshold (fallback)

    Ptr<Node> thisNode = GetNodeFromIpv4(receiver);
    if (thisNode == nullptr)
    {
        NS_LOG_WARN("PAODV: cannot map receiver to Node. Dropping RREQ.");
        return;
    }
    ...
    for (const auto &nb : m_nb.GetNeighbors())
    {
        ...
        // Prefer farther neighbors (d > threshold) as described in PAODV paper (prior neighbors)
        if (d > m_distanceThreshold)
            priorNeighbors.push_back(neighAddr);
        else
            overheadNeighbors.push_back(neighAddr);
    ...
    // Choose up to m_rreqBound neighbors, preferring priorNeighbors then overheadNeighbors
    std::vector<Ipv4Address> selected;
    uint32_t bound = m_rreqBound;

    auto pickRandomSubset = [&](std::vector<Ipv4Address> &pool, std::vector<Ipv4Address> &out, uint32_t b) {
        while (!pool.empty() && out.size() < b)
        {
            uint32_t idx = m_uniformRandomVariable->GetInteger(0, static_cast<int>(pool.size() - 1));
            out.push_back(pool[idx]);
            if (idx != pool.size() - 1)
    ...
    // If nothing selected, FALL BACK to broadcast (do not drop strictly)
    if (selected.empty())
    {
        NS_LOG_INFO("PAODV: no neighbors selected for forwarding; falling back to broadcast.");
        // broadcast (same behavior as original broadcast forward)
        for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
        {
            Ptr<Socket> socket = j->first;
            Ipv4InterfaceAddress iface = j->second;
    ...
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        ...
        // Count this batch of unicasts as ONE attempt, matching broadcast behavior
        ++m_rreqSentCount;
    }
...
RoutingProtocol::SendReply(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
    ...
    TypeHeader tHeader(PAODVTYPE_RREP);
    packet->AddHeader(tHeader);
    Ptr<Socket> socket = FindSocketWithInterfaceAddress(toOrigin.GetInterface());
    NS_ASSERT(socket);
    socket->SendTo(packet, 0, InetSocketAddress(toOrigin.GetNextHop(), PAODV_PORT));
    ++m_rrepSentCount;
}
                                             ...
                                             bool gratRep)
{
    ...
    if (gratRep)
    {
        ...
        TypeHeader type(PAODVTYPE_RREP);
        packetToDst->AddHeader(type);
        Ptr<Socket> socket = FindSocketWithInterfaceAddress(toDst.GetInterface());
        NS_ASSERT(socket);
        NS_LOG_LOGIC("Send gratuitous RREP " << packet->GetUid());
        socket->SendTo(packetToDst, 0, InetSocketAddress(toDst.GetNextHop(), PAODV_PORT));
        ++m_rrepSentCount;
    }
}
...
RoutingProtocol::SendReplyAck(Ipv4Address neighbor)
{
    ...
    RrepAckHeader h;
    TypeHeader typeHeader(PAODVTYPE_RREP_ACK);
    Ptr<Packet> packet = Create<Packet>();
    SocketIpTtlTag tag;
    tag.SetTtl(1);
    packet->AddPacketTag(tag);
    ...
    NS_ASSERT(socket);
    socket->SendTo(packet, 0, InetSocketAddress(neighbor, PAODV_PORT));
}
...
RoutingProtocol::RecvError(Ptr<Packet> p, Ipv4Address src)
{
    ...
    for (auto i = unreachable.begin(); i != unreachable.end();)
    {
        ...
        {
            TypeHeader typeHeader(PAODVTYPE_RERR);
            Ptr<Packet> packet = Create<Packet>();
            SocketIpTtlTag tag;
            tag.SetTtl(1);
            packet->AddPacketTag(tag);
                                              ...
                                              Ipv4Address origin)
{
    ...
    packet->AddPacketTag(tag);
    packet->AddHeader(rerrHeader);
    packet->AddHeader(TypeHeader(PAODVTYPE_RERR));
    ...
    else
    {
        for (auto i = m_socketAddresses.begin(); i != m_socketAddresses.end(); ++i)
        {
            ...
            socket->SendTo(packet->Copy(), 0, InetSocketAddress(destination, PAODV_PORT));
        }
    }
}
...
RoutingProtocol::DoInitialize()
{
    ...
    NS_ABORT_MSG_IF(m_ttlStart > m_netDiameter,
                    "PAODV: configuration error, TtlStart ("
...
RoutingProtocol::SelectNeighborsForRreq(const std::vector<Ipv4Address> &prior,
                                        const std::vector<Ipv4Address> &over) const
{
    std::vector<Ipv4Address> result;

    // quota = maximum number of neighbors to forward to (m_rreqBound)
    uint32_t quota = m_rreqBound;
    if (quota == 0) {
        // safe default: forward to 4 neighbors if not set
        quota = 4;
    }

    // add all high-priority up to quota (they are already considered stable/close)
    for (auto const &ip : prior) {
        if (result.size() >= quota) break;
        result.push_back(ip);
    }

    // if we still have quota, sort low-priority by distance (ascending) and add closest
    if (result.size() < quota && !over.empty()) {
        // build vector of pairs (distance, ip)
        std::vector<std::pair<double, Ipv4Address>> distIp;
        // We need 'this' node pointer for distance; if unavailable, skip distance calc
        Ptr<Node> me = m_ipv4->GetObject<Node>(); // safe if class has GetNode(); otherwise map interface to node
        for (auto const &ip : over) {
            Ptr<Node> n = GetNodeFromIpv4(ip);
            double d = std::numeric_limits<double>::infinity();
            if (me && n) {
                d = CalculateDistanceBetweenNodes(me, n);
            }
            distIp.emplace_back(d, ip);
        }
        // sort ascending by distance
        std::sort(distIp.begin(), distIp.end(),
                  [](const auto &a, const auto &b) { return a.first < b.first; });

        for (auto const &p : distIp) {
            if (result.size() >= quota) break;
