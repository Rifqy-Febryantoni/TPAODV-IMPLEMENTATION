...
#include "paodv-regression.h"

#include "bug-772.h"

#include "ns3/abort.h"
#include "ns3/paodv-helper.h"
 ...
 * @ingroup paodv-test
 *
 * @brief PAODV regression test suite
 */
class AodvRegressionTestSuite : public TestSuite
{
  public:
    AodvRegressionTestSuite()
        : TestSuite("routing-paodv-regression", Type::SYSTEM)
    {
        SetDataDir(NS_TEST_SOURCEDIR);
        // General RREQ-RREP-RRER test case
        AddTestCase(new ChainRegressionTest("paodv-chain-regression-test"),
...
} g_paodvRegressionTestSuite; ///< the test suite
...
ChainRegressionTest::ChainRegressionTest(const char* const prefix,
    ...
    : TestCase("PAODV chain regression test"),
      m_nodes(nullptr),
      m_prefix(prefix),
      m_time(t),
      m_size(size),
...
ChainRegressionTest::CreateDevices()
{
    ...
    // 2. Setup TCP/IP & PAODV
    AodvHelper paodv; // Use default parameters here
    InternetStackHelper internetStack;
    internetStack.SetRoutingHelper(paodv);
    ...
    streamsUsed += paodv.AssignStreams(*m_nodes, streamsUsed);
    // PAODV uses m_size more streams
