...
Ad Hoc On-Demand Distance Vector (TPAODV)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand
Distance Vector (TPAODV) protocol. The implementation is based on
:rfc:`3561`.

The model was written by Elena Buchatskaia and Pavel Boyko of ITTP RAS,
and is based on the ns-2 TPAODV model developed by the CMU/MONARCH group
and optimized and tuned by Samir Das and Mahesh Marina, University of
Cincinnati, and also on the TPAODV-UU implementation by Erik Nordström of
Uppsala University.

Model Description
*****************

The source code for the TPAODV model lives in the directory `src/tpaodv`.

Design
++++++

Class ``ns3::tpaodv::RoutingProtocol`` implements all functionality of
...
forwarding.  The first one, ``ns3::tpaodv::RouteOutput``, is used for
locally originated packets, and the second one, ``ns3::tpaodv::RouteInput``,
...
this functionality are attributes of ``ns3::tpaodv::RoutingProtocol``.
...
TPAODV discovers routes on demand.  Therefore, the TPAODV model buffers all
packets while a route request packet (RREQ) is disseminated.
A packet queue is implemented in tpaodv-rqueue.cc. A smart pointer to
...
* This TPAODV implementation can detect the presence of unidirectional
  ...
  This heuristic is taken from TPAODV-UU implementation and can be disabled.
...
the assertion from the TPAODV RFC that TPAODV works over UDP.  This model uses
