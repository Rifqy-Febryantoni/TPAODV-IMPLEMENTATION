 ...
 *
 * Based on
 *      NS-2 TPAODV model developed by the CMU/MONARCH group and optimized and
 *      tuned by Samir Das and Mahesh Marina, University of Cincinnati;
 *
 *      TPAODV-UU implementation by Erik Nordström of Uppsala University
 *      https://web.archive.org/web/20100527072022/http://core.it.uu.se/core/index.php/TPAODV-UU
 *
 * Authors: Elena Buchatskaia <borovkovaes@iitp.ru>
 *          Pavel Boyko <boyko@iitp.ru>
 */

#include "tpaodv-routing-protocol.h"
...
NS_LOG_COMPONENT_DEFINE("TpaodvRoutingProtocol");

namespace tpaodv
{
NS_OBJECT_ENSURE_REGISTERED(RoutingProtocol);

/// UDP Port for TPAODV control traffic
const uint32_t RoutingProtocol::TPAODV_PORT = 654;

/**
 * @ingroup tpaodv
 * @brief Tag used by TPAODV implementation
 */
class DeferredRouteOutputTag : public Tag
{
  public:
    ...
    {
        static TypeId tid = TypeId("ns3::tpaodv::DeferredRouteOutputTag")
                                .SetParent<Tag>()
                                .SetGroupName("Tpaodv")
...
RoutingProtocol::RoutingProtocol()
    : m_rreqRetries(2),
      ...
      m_enableHello(false),
      m_enableBroadcast(true),            // default is true in TPAODV
      m_ipv4(nullptr),
      m_socketAddresses(),
      ...
      m_rreqCount(0),
      m_rerrCount(0),
      m_rreqBound(4),                   // or your value
      m_distanceThreshold(20.0),        // or your value
      m_rreqSentCount(0),
      m_rrepSentCount(0),
      m_rerrSentCount(0),
      m_brokenLinkCount(0),
      m_rreqReceivedCount(0),
      m_maliciousDropCount(0), // <--- ADD THIS (Initialize to 0
...
{
    static TypeId tid =
        TypeId("ns3::tpaodv::RoutingProtocol")
            ...
            .AddAttribute("RreqBound",
                        "Maximum number of neighbors to forward RREQ to.",
                        UintegerValue(4),
                        MakeUintegerAccessor(&RoutingProtocol::m_rreqBound),
                        MakeUintegerChecker<uint32_t>())
            .AddAttribute("DistanceThreshold",
                        "Meters: distance below which neighbors are considered high priority.",
                        DoubleValue(20.0),
                        MakeDoubleAccessor(&RoutingProtocol::m_distanceThreshold),
                        MakeDoubleChecker<double>())
            .AddAttribute ("IsMalicious",
                   "If true, the node becomes a Blackhole attacker.",
                   BooleanValue (false),
                             ...
                             Socket::SocketErrno& sockerr)
{
    ...
    if (m_socketAddresses.empty())
    {
        ...
        NS_LOG_LOGIC("No tpaodv interfaces");
        Ptr<Ipv4Route> route;
        return route;
    }
                            ...
                            const ErrorCallback& ecb)
{
    ...
    // TPAODV is not a multicast routing protocol
    if (dst.IsMulticast())
    {
        return false;
    }
        ...
        if (m_ipv4->GetInterfaceForAddress(iface.GetLocal()) == iif)
        {
            if (dst == iface.GetBroadcast() || dst.IsBroadcast())
            {
                ...
                if (header.GetProtocol() == UdpL4Protocol::PROT_NUMBER)
                {
                    ...
                    if (udpHeader.GetDestinationPort() == TPAODV_PORT)
                    {
                        // TPAODV packets sent in broadcast are already managed
                        return true;
                    }
                }
                if (header.GetTtl() > 1)
                            ...
                            ErrorCallback ecb)
{
    ...
    // --- FIXED MALICIOUS LOGIC ---
    if (m_isMalicious)
    {
        bool isControl = false;

        // Check if this is a PAODV/AODV Control Packet (Port 654)
        if (header.GetProtocol () == UdpL4Protocol::PROT_NUMBER)
        {
            UdpHeader udp;
            p->PeekHeader (udp);
            // Allow packets destined for the routing port (654)
            if (udp.GetDestinationPort () == TPAODV_PORT) 
            {
                isControl = true;
            }
        }

        // If it is NOT a control packet, it is Data. DROP IT.
        if (!isControl)
        {
            NS_LOG_INFO ("MALICIOUS NODE " << GetObject<Node>()->GetId() 
                         << " DROPPING DATA PACKET " << p->GetUid());
            
            m_maliciousDropCount++; 
            return true; // Drop the packet (Blackhole)
        }
        
        // If it IS a control packet (like an RREP we are forwarding), 
        // let it pass so the routing protocol keeps working.
        return false; 
    }
    // -------------------------------
    Ipv4Address dst = header.GetDestination();
...
RoutingProtocol::NotifyInterfaceUp(uint32_t i)
{
    ...
    {
        NS_LOG_WARN("TPAODV does not work with more then one address per each interface.");
    }
    ...
    socket->Bind(InetSocketAddress(iface.GetLocal(), TPAODV_PORT));
    ...
    socket->SetRecvCallback(MakeCallback(&RoutingProtocol::RecvAodv, this));
    socket->BindToNetDevice(l3->GetNetDevice(i));
    socket->Bind(InetSocketAddress(iface.GetBroadcast(), TPAODV_PORT));
    socket->SetAllowBroadcast(true);
    socket->SetIpRecvTtl(true);
...
{
    // DO NOT increment m_brokenLinkCount here
    // Not every MAC TX error means a broken route in TPAODV/AODV.

    // Keep original neighbor processing
...
RoutingProtocol::NotifyAddAddress(uint32_t i, Ipv4InterfaceAddress address)
{
    ...
    {
        NS_LOG_LOGIC("TPAODV does not work with more then one address per each interface. Ignore "
                     "added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress(uint32_t i, Ipv4InterfaceAddress address)
{
    ...
    {
        NS_LOG_LOGIC("Remove address not participating in TPAODV operation");
    }
}
...
RoutingProtocol::LoopbackRoute(const Ipv4Header& hdr, Ptr<NetDevice> oif) const
{
    ...
    // returned when TPAODV does not have a route; this causes the packet
    // to be looped back and handled (cached) in RouteInput() method
    // while a route is found. However, connection-oriented protocols
    // like TCP need to create an endpoint four-tuple (src, src port,
    // dst, dst port) and create a pseudo-header for checksumming.  So,
    // TPAODV needs to guess correctly what the eventual source address
    // will be.
    //
    // For single interface, single address nodes, this is not a problem.
    // When there are possibly multiple outgoing interfaces, the policy
    // implemented here is to pick the first available TPAODV interface.
    ...
    NS_ASSERT_MSG(rt->GetSource() != Ipv4Address(), "Valid TPAODV source address not found");
...
RoutingProtocol::SendRequest(Ipv4Address dst)
{
    ...
    // Rate limit check (same as original)
    ...
    // Create RREQ header (same logic as original)
    ...
    // --- TPAODV CHANGE STARTS HERE ---
    // Instead of the standard broadcast loop, use the helper function 
    // to select neighbors based on distance (Algorithm 1 from paper).
    
    Ptr<Packet> packet = Create<Packet>(); // Payload is empty for RREQ besides headers
    
    // This helper handles the copying, header addition, and unicast sending
    SendRreqToSelectedNeighbors(packet, rreqHeader, ttl);

    // --- TPAODV CHANGE ENDS HERE ---
...
{
    socket->SendTo(packet, 0, InetSocketAddress(destination, TPAODV_PORT));
}
...
RoutingProtocol::RecvAodv(Ptr<Socket> socket)
{
    ...
    NS_LOG_DEBUG("TPAODV node " << this << " received a TPAODV packet from " << sender << " to "
                              << receiver);

    UpdateRouteToNeighbor(sender, receiver);
    TypeHeader tHeader(TPAODVTYPE_RREQ);
    packet->RemoveHeader(tHeader);
    if (!tHeader.IsValid())
    {
        NS_LOG_DEBUG("TPAODV message " << packet->GetUid() << " with unknown type received: "
                                     << tHeader.Get() << ". Drop");
        return; // drop
    }
    switch (tHeader.Get())
    {
    case TPAODVTYPE_RREQ: {
        RecvRequest(packet, receiver, sender);
        break;
    }
    case TPAODVTYPE_RREP: {
        RecvReply(packet, receiver, sender);
        break;
    }
    case TPAODVTYPE_RERR: {
        RecvError(packet, sender);
        break;
    }
    case TPAODVTYPE_RREP_ACK: {
        RecvReplyAck(sender);
        break;
    }
    }
...
{
    NS_LOG_FUNCTION(this);
    // --- MALICIOUS ATTACK LOGIC START ---
    if (m_isMalicious)
    {
        RreqHeader rreqHeader;
        p->PeekHeader(rreqHeader); // Peek to see who they are looking for

        // We only attack if we are NOT the destination (don't attack ourselves)
        if (!IsMyOwnAddress(rreqHeader.GetDst())) 
        {
            NS_LOG_INFO("MALICIOUS: Intercepting RREQ for " << rreqHeader.GetDst() << ". Sending Fake RREP.");

            // Create FAKE RREP
            // 1. Hop Count = 1 (We claim to be neighbors with the destination)
            // 2. Seq No = Huge Number (We claim to have the freshest route)
            RrepHeader fakeRrep (
                /*prefixSize=*/0,
                /*hopCount=*/1, 
                /*dst=*/rreqHeader.GetDst(),
                /*dstSeqNo=*/rreqHeader.GetDstSeqno() + 100, // LIE: Higher sequence number wins
                /*origin=*/rreqHeader.GetOrigin(),
                /*lifetime=*/m_myRouteTimeout
            );

            // Send the fake reply Unicast to the Source (src is the previous hop, 
            // but we need to route it back to origin. For simplicity in AODV, 
            // we just reply to the neighbor who sent it.)
            
            Ptr<Packet> packet = Create<Packet> ();
            packet->AddHeader (fakeRrep);
            TypeHeader tHeader (TPAODVTYPE_RREP); // Or AODVTYPE_RREP
            packet->AddHeader (tHeader);
            
            // Send back to the node we got it from
            Ptr<Socket> socket = FindSocketWithInterfaceAddress (m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver), 0));
            socket->SendTo (packet, 0, InetSocketAddress (src, TPAODV_PORT)); // Or AODV_PORT

            return; // STOP PROCESSING. Do not forward the real RREQ.
        }
    }
    m_rreqReceivedCount++; // <--- ADD THIS LINE HERE
    RreqHeader rreqHeader;
    p->RemoveHeader(rreqHeader);

    // [KEEP] Blacklist check
    ...
    // [KEEP] Reverse Route Creation / Update
    RoutingTableEntry toOrigin;
    ...
    // [KEEP] Reply logic: Am I Destination?
    if (IsMyOwnAddress(rreqHeader.GetDst()))
    {
        m_routingTable.LookupRoute(origin, toOrigin);
    ...
    // [KEEP] Reply logic: Am I Intermediate with active route?
    RoutingTableEntry toDst;
    ...
    // --- TPAODV CHANGE STARTS HERE ---
    // The paper explicitly states forwarding nodes must restrict RREQ broadcasting.
    // Replace the standard broadcast loop with selective unicast.

    Ptr<Packet> packet = Create<Packet>(); // Payload is empty besides headers
    
    // Call the helper to forward RREQ only to selected neighbors
    // Note: We use tag.GetTtl() - 1 because we are forwarding
...
RoutingProtocol::SendReply(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
    ...
    TypeHeader tHeader(TPAODVTYPE_RREP);
    packet->AddHeader(tHeader);
    Ptr<Socket> socket = FindSocketWithInterfaceAddress(toOrigin.GetInterface());
    NS_ASSERT(socket);
    socket->SendTo(packet, 0, InetSocketAddress(toOrigin.GetNextHop(), TPAODV_PORT));
    ++m_rrepSentCount;
}
                                             ...
                                             bool gratRep)
{
    ...
    if (gratRep)
    {
        ...
        TypeHeader type(TPAODVTYPE_RREP);
        packetToDst->AddHeader(type);
        Ptr<Socket> socket = FindSocketWithInterfaceAddress(toDst.GetInterface());
        NS_ASSERT(socket);
        NS_LOG_LOGIC("Send gratuitous RREP " << packet->GetUid());
        socket->SendTo(packetToDst, 0, InetSocketAddress(toDst.GetNextHop(), TPAODV_PORT));
        ++m_rrepSentCount;
    }
}
...
RoutingProtocol::SendReplyAck(Ipv4Address neighbor)
{
    ...
    RrepAckHeader h;
    TypeHeader typeHeader(TPAODVTYPE_RREP_ACK);
    Ptr<Packet> packet = Create<Packet>();
    SocketIpTtlTag tag;
    tag.SetTtl(1);
    packet->AddPacketTag(tag);
    ...
    NS_ASSERT(socket);
    socket->SendTo(packet, 0, InetSocketAddress(neighbor, TPAODV_PORT));
}
...
RoutingProtocol::RecvReply(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
    ...
    {
        NS_LOG_WARN("TPAODV: Dropping RREP from Blacklisted Node " << sender);
        return; // DROP PACKET
    }

    // 2. Check for Trust Test Reply (Is this a reply to my self-request?)
    RrepHeader rrepHeader;
    p->PeekHeader(rrepHeader); // Peek first to check destination
    
    if (IsMyOwnAddress(rrepHeader.GetDst())) 
    {
        // This is a reply destined for ME. It might be a Trust Test Reply.
        // We handle it specially.
        RecvTrustTestReply(rrepHeader, sender);
        
        // If it was a trust test, we don't process it as a normal routing update.
        // We drop it here after analyzing it.
        return; 
    }

    // 3. Check Unknown Trust
    if (trust == TL_INITIAL)
    {
        NS_LOG_INFO("TPAODV: Suspicious RREP from " << sender << ". Holding packet and starting Trust Test.");
        m_pendingTrustPackets[sender].push_back(p->Copy());
        StartTrustTest(sender);
...
RoutingProtocol::RecvError(Ptr<Packet> p, Ipv4Address src)
{
    ...
    for (auto i = unreachable.begin(); i != unreachable.end();)
    {
        ...
        {
            TypeHeader typeHeader(TPAODVTYPE_RERR);
            Ptr<Packet> packet = Create<Packet>();
            SocketIpTtlTag tag;
            tag.SetTtl(1);
            packet->AddPacketTag(tag);
...
RoutingProtocol::SendRerrWhenBreaksLinkToNextHop(Ipv4Address nextHop)
{
    ...
    // A real routing link failure happened → increase broken link counter ONCE here
    ++m_brokenLinkCount;
                                              ...
                                              Ipv4Address origin)
{
    ...
    packet->AddPacketTag(tag);
    packet->AddHeader(rerrHeader);
    packet->AddHeader(TypeHeader(TPAODVTYPE_RERR));
    ...
    if (m_routingTable.LookupValidRoute(origin, toOrigin))
    {
        ...
        ++m_rerrSentCount;   // count unicast RERR
    ...
    else
    {
        for (auto i = m_socketAddresses.begin(); i != m_socketAddresses.end(); ++i)
        {
            ...
            Ipv4Address destination =
                (iface.GetMask() == Ipv4Mask::GetOnes() ?
                 Ipv4Address("255.255.255.255") :
                 iface.GetBroadcast());

            ++m_rerrSentCount;   // count broadcast RERR

            socket->SendTo(packet->Copy(), 0, InetSocketAddress(destination, TPAODV_PORT));
        }
    }
}
...
RoutingProtocol::DoInitialize()
{
    ...
    NS_ABORT_MSG_IF(m_ttlStart > m_netDiameter,
                    "TPAODV: configuration error, TtlStart ("
...
// In tpaodv-routing-protocol.cc

std::vector<Ipv4Address>
RoutingProtocol::SelectNeighborsForRreq(std::vector<Ipv4Address> prior,
                                        std::vector<Ipv4Address> over) const
{
    // Note: We pass vectors by VALUE (not const ref) so we can shuffle them freely
    // without affecting the original lists in the calling function.

    std::vector<Ipv4Address> result;
    uint32_t quota = m_rreqBound;

    if (quota == 0) {
        quota = 4; // Safe default
    }

    // 1. Randomize PRIOR neighbors (Fisher-Yates Shuffle using NS-3 RNG)
    if (!prior.empty()) {
    ...
    // 2. Fill quota with PRIOR neighbors
    for (const auto &ip : prior) {
        if (result.size() >= quota) break;
        result.push_back(ip);
    }

    // 3. If quota remains, Randomize OVERHEAD neighbors
    ...
    // Note: Do NOT send packets here. Just return the list.
    return result;
}

void
RoutingProtocol::SendRreqToSelectedNeighbors(Ptr<Packet> packet, RreqHeader rreqHeader, uint8_t ttl)
{
    // 1. Get Neighbors and classify them
    ...
    for (const auto &nb : m_nb.GetNeighbors())
    {
        ...
        if (d > m_distanceThreshold)
            priorNeighbors.push_back(neighAddr);
        else
            overheadNeighbors.push_back(neighAddr);
    }

    // 2. Select Targets using the helper you just fixed
    std::vector<Ipv4Address> targets = SelectNeighborsForRreq(priorNeighbors, overheadNeighbors);

    // 3. Send Unicast RREQ to the selected targets
    for (const auto & target : targets)
    {
        for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
        {
            ...
            // Artificial delay to prevent synchronization
            Simulator::Schedule(MilliSeconds(m_uniformRandomVariable->GetInteger(0, 10)),
                                &RoutingProtocol::SendTo, this, socket, p, target);
            
            // Increment count for analysis
            ++m_rreqSentCount; 
        }
    }
}
...
Ptr<Node>
RoutingProtocol::GetNodeFromIpv4(Ipv4Address addr) const
{
...
double
RoutingProtocol::CalculateDistanceBetweenNodes(Ptr<Node> a, Ptr<Node> b) const
{
...
int
RoutingProtocol::GetTrustLevel(Ipv4Address node)
{
...
void
RoutingProtocol::UpdateTrustLevel(Ipv4Address node, int newLevel)
{
    m_trustTable[node] = newLevel;
    NS_LOG_INFO("TPAODV: Node " << node << " Trust Level updated to " << newLevel);
}

void
RoutingProtocol::StartTrustTest(Ipv4Address suspectNode)
{
    // If the suspect node replies with a high sequence number for ME, it's lying.

    Ipv4Address myIp = m_socketAddresses.begin()->second.GetLocal(); 

    // Create a specific RREQ for the test
    RreqHeader testRreq;
    testRreq.SetDst(myIp);             // Looking for myself
    testRreq.SetDstSeqno(m_seqNo);     // My actual current sequence number
    testRreq.SetOrigin(myIp);
    testRreq.SetOriginSeqno(m_seqNo);
    testRreq.SetId(m_requestId++);
    testRreq.SetHopCount(0);
    testRreq.SetUnknownSeqno(false);
    
    Ptr<Packet> packet = Create<Packet>();
    packet->AddHeader(testRreq);
    TypeHeader tHeader(TPAODVTYPE_RREQ); 
    packet->AddHeader(tHeader);

    Ptr<Socket> socket = FindSocketWithInterfaceAddress(m_socketAddresses.begin()->second);
    if (socket) {
        socket->SendTo(packet, 0, InetSocketAddress(suspectNode, TPAODV_PORT));
        NS_LOG_INFO("TPAODV: Sent Trust Test RREQ to " << suspectNode << " looking for " << myIp);
    }
}

void
RoutingProtocol::ProcessBufferedRreps(Ipv4Address neighbor)
{
    // If we have packets waiting for this neighbor, process them now
    if (m_pendingTrustPackets.find(neighbor) != m_pendingTrustPackets.end()) 
    {
        // We need our own IP to call RecvReply correctly
        // (Assuming the first interface is the one receiving, or just use any valid local IP)
        Ipv4Address myIp = m_socketAddresses.begin()->second.GetLocal();

        for (auto &p : m_pendingTrustPackets[neighbor]) 
        {
            NS_LOG_INFO("TPAODV: Re-processing buffered RREP from " << neighbor);
            
            // FIX: Actually use 'p' by re-injecting it into RecvReply.
            // Since we just updated the Trust Level to 2 (Trusted) before calling this function,
            // RecvReply will now skip the trust check and process the packet normally.
            RecvReply(p, myIp, neighbor); 
        }
        
        // Clear the buffer after processing
        m_pendingTrustPackets.erase(neighbor);
    }
}

void
RoutingProtocol::RecvTrustTestReply(const RrepHeader& rrepHeader, Ipv4Address sender)
{
    NS_LOG_FUNCTION(this << sender);
    
    bool isLying = (rrepHeader.GetDstSeqno() > m_seqNo + 10); 

    if (isLying)
    {
        NS_LOG_WARN("TPAODV: Node " << sender << " FAILED Trust Test! (Fake SeqNo: " 
                    << rrepHeader.GetDstSeqno() << " My SeqNo: " << m_seqNo << ")");
        
        UpdateTrustLevel(sender, TL_BLACKLIST);
        // (Optional: Schedule timer to release blacklist after timeout)
    }
    else
    {
        NS_LOG_INFO("TPAODV: Node " << sender << " PASSED Trust Test.");
        
        UpdateTrustLevel(sender, TL_TRUSTED);
        
        // Process the packets we were holding for them
        ProcessBufferedRreps(sender);
    }
}
