...
RoutingProtocol::RoutingProtocol()
    : m_rreqRetries(2),
      ...
      m_netTraversalTime((2 * m_netDiameter) * m_nodeTraversalTime),
      m_pathDiscoveryTime(2 * m_netTraversalTime),
      m_myRouteTimeout(2 * std::max(m_pathDiscoveryTime, m_activeRouteTimeout)),
...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    ...
    if (m_isMalicious)
    {
        ...
        if (!IsMyOwnAddress(rreqHeader.GetDst())) 
        {
            ...
            TypeHeader tHeader (AODVTYPE_RREP); // Or AODVTYPE_RREP
            packet->AddHeader (tHeader);
            ...
            socket->SendTo (packet, 0, InetSocketAddress (src, AODV_PORT)); // Or AODV_PORT
