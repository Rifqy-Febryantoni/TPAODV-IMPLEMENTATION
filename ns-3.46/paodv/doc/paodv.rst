...
Ad Hoc On-Demand Distance Vector (PAODV)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand
Distance Vector (PAODV) protocol. The implementation is based on
:rfc:`3561`.

The model was written by Elena Buchatskaia and Pavel Boyko of ITTP RAS,
and is based on the ns-2 PAODV model developed by the CMU/MONARCH group
and optimized and tuned by Samir Das and Mahesh Marina, University of
Cincinnati, and also on the PAODV-UU implementation by Erik Nordström of
Uppsala University.

Model Description
*****************

The source code for the PAODV model lives in the directory `src/paodv`.

Design
++++++

Class ``ns3::paodv::RoutingProtocol`` implements all functionality of
...
forwarding.  The first one, ``ns3::paodv::RouteOutput``, is used for
locally originated packets, and the second one, ``ns3::paodv::RouteInput``,
...
this functionality are attributes of ``ns3::paodv::RoutingProtocol``.
...
PAODV discovers routes on demand.  Therefore, the PAODV model buffers all
packets while a route request packet (RREQ) is disseminated.
A packet queue is implemented in paodv-rqueue.cc. A smart pointer to
...
* This PAODV implementation can detect the presence of unidirectional
  ...
  This heuristic is taken from PAODV-UU implementation and can be disabled.
...
the assertion from the PAODV RFC that PAODV works over UDP.  This model uses
