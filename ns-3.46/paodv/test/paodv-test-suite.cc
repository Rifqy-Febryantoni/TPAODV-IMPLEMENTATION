 ...
 */
#include "ns3/paodv-neighbor.h"
#include "ns3/paodv-packet.h"
#include "ns3/paodv-rqueue.h"
#include "ns3/paodv-rtable.h"
...
{
    TypeHeaderTest()
        : TestCase("PAODV TypeHeader")
    {
    }

    void DoRun() override
    {
        TypeHeader h(PAODVTYPE_RREQ);
        NS_TEST_EXPECT_MSG_EQ(h.IsValid(), true, "Default header is valid");
        NS_TEST_EXPECT_MSG_EQ(h.Get(), PAODVTYPE_RREQ, "Default header is RREQ");

        Ptr<Packet> p = Create<Packet>();
        p->AddHeader(h);
        TypeHeader h2(PAODVTYPE_RREP);
 ...
 * @ingroup paodv-test
 *
 * @brief Unit test for PAODV routing table entry
 */
 ...
 * @ingroup paodv-test
 *
 * @brief Unit test for PAODV routing table
 */
 ...
 * @ingroup paodv-test
 *
 * @brief PAODV test suite
 */
class AodvTestSuite : public TestSuite
{
  public:
    AodvTestSuite()
        : TestSuite("routing-paodv", Type::UNIT)
    {
...
} g_paodvTestSuite; ///< the test suite
